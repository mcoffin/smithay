...
fn run_winit<G, F>(init_fn: F)
where
    G: WinitGraphics + 'static,
    <G as WinitGraphics>::Renderer: ImportDmaWl + ImportMemWl + ImportEgl,
    ...
    F: FnOnce() -> Result<(WinitGraphicsBackend<G>, WinitEventLoop), WinitError>,
{
...
//             #[cfg(feature = "debug")]
//             let mut renderdoc = state.renderdoc.as_mut();
//             let render_res = backend.bind(From::from).and_then(|_| {
//                 #[cfg(feature = "debug")]
//                 if let Some(renderdoc) = renderdoc.as_mut() {
//                     renderdoc.start_frame_capture(
//                         backend.renderer().egl_context().get_context_handle(),
...
// }
pub fn run_winit_gles() {
    run_winit::<WinitEglGraphics<GlesRenderer>, _>(winit::init);
}

pub fn run_winit_vulkan() {
    todo!()
    // run_winit::<WinitVulkanGraphics, _>(smithay::backend::winit::init_vulkan);
}
// pub fn run_winit_vulkan() {
//     // let mut event_loop = EventLoop::try_new().unwrap();
//     // let display = Display::new().unwrap();
